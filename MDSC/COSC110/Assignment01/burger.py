...
total_price  = (order_number -1)*5
for i in order:
    if bun_type == 'gluten free':
        total_price += 1
    if patty_option > 1:
        total_price += (patty_option-1)*3
    if cheese_option > 1:
        total_price += (cheese_option-1)
    if tomato_option == 'y' or 'yes':
        total_price + 1
    if lettuce_option == 'y' or 'yes':
        total_price + 1
    if onion_option == 'y' or 'yes':
        total_price + 1
